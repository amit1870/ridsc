...
def is_up_to_date():
    ...
    sha_file = path.join(settings.get_configdir(), 'latest_rids_sha')
...
def template(template_name, **context):
    """RIDS template response generator. Shares the
...
@route('/system_info')
def system_info():
    viewer_log_file = '/tmp/rids_viewer.log'
    if path.exists(viewer_log_file):
        viewlog = check_output(['tail', '-n', '20', viewer_log_file]).split('\n')
    else:
        viewlog = ["(no viewer log present -- is only the rids server running?)\n"]
