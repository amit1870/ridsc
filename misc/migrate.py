...
configdir = os.path.join(os.getenv('HOME'), '.ridsc/')
database = os.path.join(configdir, 'rids.db')
...
def ensure_conf():
    ...
    conf_file = os.path.join(os.getenv('HOME'), '.ridsc', 'rids.conf')
    if not os.path.isfile(conf_file):
        print "Copying in config file..."
        example_conf = os.path.join(os.getenv('HOME'), 'rids', 'misc', 'rids.conf')
        shutil.copy(example_conf, conf_file)


def fix_supervisor():
    incorrect_supervisor_symlink = '/etc/supervisor/conf.d/supervisor_rids.conf'
    ...
    supervisor_symlink = '/etc/supervisor/conf.d/rids.conf'
    old_target = '/home/pi/ridsc/misc/rids.conf'
    new_target = '/home/pi/ridsc/misc/supervisor_rids.conf'
