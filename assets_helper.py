...
get_day  = datetime.datetime.now().strftime('%a')
days = ['Mon','Tue','Wed','Thu','Fri','Sat','Sun']  # Mon is first of week
idx = days.index(get_day)
...
def is_active(asset, at_time=None):
    ...
    >>> asset = {'asset_id': u'4c8dbce552edb5812d3a866cfe5f159d', 'mimetype': u'web', 'name': u'Rigvee', 'end_date': datetime.datetime(2013, 1, 19, 23, 59), 'uri': u'http://www.rigvee.net', 'duration': u'5', 'is_enabled': True, 'nocache': 0, 'play_order': 1, 'start_date': datetime.datetime(2013, 1, 16, 0, 0)};
    ...
    if asset['is_enabled'] and asset['start_date'] and asset['end_date'] and int(asset['show_days'][idx]):
...
def read(conn, asset_id=None, keys=FIELDS):
    ...
    json_data.close()
    [asset.update({'start_date': parse(asset['start_date'],ignoretz=True),'end_date': parse(asset['end_date'],ignoretz=True),'asset_id':str(asset['asset_id'])}) for asset in assets]
    return assets
